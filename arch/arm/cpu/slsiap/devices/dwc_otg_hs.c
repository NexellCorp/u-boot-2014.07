...
static void s3c_usb_read_out_fifo(u8 *buf, int num);
...
static void s3c_usb_core_soft_reset(void)
{
	u32 tmp;
...
static void s3c_usb_wait_cable_insert(void)
{
	u32 tmp;
	int ucFirst=1;
...
static void s3c_usb_init_core(void)
{
...
static void s3c_usb_check_current_mode(u8 *pucMode)
{
	u32 tmp;
...
static void s3c_usb_set_soft_disconnect(void)
{
	u32 tmp;
...
static void s3c_usb_clear_soft_disconnect(void)
{
	u32 tmp;
...
static void s3c_usb_init_device(void)
{
...
#ifdef USB_OTG_DEBUG_SETUP
static void s3c_usb_print_pkt(u8 *pt, u8 count)
{
	int i;
...
static void s3c_usb_verify_checksum(void)
{
	u8 *cs_start, *cs_end;
	u16 dnCS;
	u16 checkSum;
...
static void s3c_usb_write_ep0_fifo(u8 *buf, int num)
{
	int i;
	u32 Wr_Data=0;
...
static void s3c_usb_get_desc(void)
{
...
static void s3c_usb_clear_feature(void)
{
...
static void s3c_usb_get_status(void)
{
	switch(otg.dev_req.bmRequestType) {
	case  (0x80):	/*device */
		DBG_SETUP1("DEVICE\n");
...
static void s3c_usb_ep0_int_hndlr(void)
{
	u16 i;
	u32 buf[2]={0x0000, };
	u16 addr;
...
static void s3c_usb_transfer_ep0(void)
{
...
static void s3c_usb_upload_start(void)
{
	u8 tmp_buf[12];
	u32 check;
...
static void s3c_usb_download_start(u32 fifo_cnt_byte)
{
	u8 tmp_buf[8];
...
static void s3c_usb_download_continue(u32 fifo_cnt_byte)
{
	if (otg.op_mode == USB_CPU) {
...
static void s3c_usb_int_bulkout(u32 fifo_cnt_byte)
{
...
static void s3c_usb_dma_in_done(void)
{
	s32 remain_cnt;
...
static void s3c_usb_dma_out_done(void)
{
	s32 remain_cnt;
...
static void s3c_usb_set_all_outep_nak(void)
{
	u8 i;
	u32 tmp;
...
static void s3c_usb_clear_all_outep_nak(void)
{
	u8 i;
	u32 tmp;
...
static void s3c_usb_set_max_pktsize(USB_SPEED speed)
{
	if (speed == USB_HIGH)
	{
		otg.speed = USB_HIGH;
...
static void s3c_usb_set_endpoint(void)
{
...
static void s3c_usb_set_descriptors(void)
{
	/* Standard device descriptor */
...
#if 0
static void s3c_usb_check_speed(USB_SPEED *speed)
{
	u32 status;
...
static void s3c_usb_clear_upfile_info(void)
{
	otg.up_addr= 0;
	otg.up_size= 0;
	otg.up_ptr = 0;
...
static int s3c_usb_check_setconf(void)
{
	if (otg.set_config == 0)
		return FALSE;
	else
...
static void s3c_usb_clear_dnfile_info(void)
{
	otg.dn_addr = 0;
	otg.dn_filesize = 0;
	otg.dn_ptr = 0;
...
static void s3c_usb_set_opmode(USB_OPMODE mode)
{
	otg.op_mode = mode;
...
static void s3c_usb_reset(void)
{
...
static int s3c_usb_set_init(void)
{
	u32 status;
...
static void dwc_otg_save_ep0_state(U32 rx_status);
static void s3c_usb_pkt_receive(void)
{
	u32 rx_status;
	u32 fifo_cnt_byte;
...
static void s3c_usb_transfer(void)
{
	u32 ep_int;
	u32 check_dma;
	u32 ep_int_status;
