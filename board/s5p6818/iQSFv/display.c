...
while(1) {
	...
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LCD_GD_PWR_EN), PAD_GET_BITNO(CFG_IO_LCD_GD_PWR_EN), CFALSE);

	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LCD_BL_ENB), PAD_GET_BITNO(CFG_IO_LCD_BL_ENB), CFALSE);
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LCD_TEST), PAD_GET_BITNO(CFG_IO_LCD_TEST), CFALSE);

	printf(" PAD TEST A0, A26, A27, C28 , D1 On Off\n");
}
#endif 

#if defined(CONFIG_DISPLAY_OUT_LVDS)
#if defined(CONFIG_BOOT_LOGO_ENABLE)
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LVDS2RGB_EN), PAD_GET_BITNO(CFG_IO_LVDS2RGB_EN), CTRUE);
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LCD_GD_PWR_EN), PAD_GET_BITNO(CFG_IO_LCD_GD_PWR_EN), CTRUE);
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LCD_PWR_ENB), PAD_GET_BITNO(CFG_IO_LCD_PWR_ENB), CTRUE);	
	NX_GPIO_SetOutputValue(PAD_GET_GROUP(CFG_IO_LCD_RESET), PAD_GET_BITNO(CFG_IO_LCD_RESET), CTRUE);
#endif
