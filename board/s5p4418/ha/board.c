...
static void bd_gpio_init(void)
{
	...
	/* GPIO pad function */
	for (index = 0; NUMBER_OF_GPIO_MODULE-1 > index; index++) {
...
{
#if 0 //defined(CONFIG_SYS_MMC_BOOT_DEV)
	char boot[16];
	sprintf(boot, "mmc dev %d", CONFIG_SYS_MMC_BOOT_DEV);
	run_command(boot, 0);
#endif

#if 0 // reboot bootloader -> fastboot(download)
...
#ifdef CONFIG_NXC100_DOWNLOAD
	//run_command("mmc read 0x49000000 0x8000 0x42", 0);
	//run_command("nxc100 0x49000000", 0);
	run_command("nxc100 3", 0);
#endif
