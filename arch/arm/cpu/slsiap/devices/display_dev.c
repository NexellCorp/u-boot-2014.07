...
static U32 mlc_gammatable[256] = // gamma table 1p20
{
0	,
1	,
3	,
...
void disp_multily_enable(int module, int layer, int enable)
{
	...
	/* Gamma setting */
     ...
     for(i=0; i<256; i++) {
		p_nx_mlc_gammatable->R_TABLE[i] = mlc_gammatable[i];
		p_nx_mlc_gammatable->G_TABLE[i] = mlc_gammatable[i];
		p_nx_mlc_gammatable->B_TABLE[i] = mlc_gammatable[i];
	}
