...
#define DOS_EBR_BLOCK				(0x100000/NAND_BLOCK_SIZE)
...
static inline void part_nand_chs(dos_partition_t *pt, lbaint_t lba_start, lbaint_t lba_size)
{
...
static int nand_get_part_table_extended(block_dev_desc_t *desc,
				lbaint_t lba_start, lbaint_t relative, uint64_t (*parts)[2], int *part_num)
{
	...
	if (0 > desc->block_read(desc->dev, lba_start, 1, (void *)buffer)) {
		printf ("** Error read nand.%d partition info **\n", desc->dev);
		return -1;
	}
	...
	for (i = 0; 4 > i; i++, pt++) {
		if (is_extended(pt->sys_ind)) {
			...
			nand_get_part_table_extended(desc,
...
int nand_get_part_table(block_dev_desc_t *desc, uint64_t (*parts)[2], int *part_num)
{
	int partcnt = 0;
	int ret = nand_get_part_table_extended(desc, 0, 0, parts, &partcnt);
	if (0 > ret)
		return ret;
...
int nand_make_part_table_extended(block_dev_desc_t *desc, lbaint_t lba_start,
					lbaint_t relative, uint64_t (*parts)[2], int part_num)
{
	...
	if (part_num)
		return nand_make_part_table_extended(desc,
				lba_s + relative, relative, &parts[1], part_num-1);

	return ret;
}

int nand_make_part_table(block_dev_desc_t *desc,
				uint64_t (*parts)[2], int part_num, unsigned int part_type)
{
	unsigned char buffer[NAND_BLOCK_SIZE];
	dos_partition_t *pt;
	uint64_t lba_s, lba_l, last_lba = 0;
	uint64_t available = desc->lba;
	int part_tables = part_num, part_EBR = 0;
	int i = 0, ret = 0;

	debug("--- Create nand.%d partitions %d ---\n", desc->dev, part_num);
	...
	debug("Total = %lld * %d :0x%llx (%d.%d G) \n", available,
		(int)desc->blksz, (uint64_t)(available)*desc->blksz,
		(int)((available*desc->blksz)/(1024*1024*1024)),
		(int)((available*desc->blksz)%(1024*1024*1024)));
	...
	if (part_EBR)
		return nand_make_part_table_extended(desc, lba_s, lba_s, &parts[part_tables], part_num - part_tables);
...
/*
 * cmd : ndisk 0 n, s1:size, s2:size, s3:size, s4:size
 */
static int do_ndisk(cmd_tbl_t *cmdtp, int flag, int argc, char* const argv[])
{
	...
	ret = get_device("nand", argv[1], &desc);
	if (ret < 0) {
		printf ("** Not find device nand.%s **\n", argv[1]);
		return 1;
	}
	...
	if (2 == argc) {
		...
		nand_make_part_table(desc, parts, count, PART_TYPE_DOS);
	}
	return 0;
}
...
U_BOOT_CMD(
	ndisk, 16, 1, do_ndisk,
	"ndisk list or create ms-dos partition tables (MAX TABLE 7)",
	"<dev no>\n"
