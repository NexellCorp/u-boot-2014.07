...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	dwc_otg_cil_init(&g_core_if, (uint32_t*)PHY_BASEADDR_HSOTG);
		    ...
		    int length)
{
	...
	if (length > DWC_OTG_HCD_DATA_BUF_SIZE) {
		printf("submit_bulk_msg: %d is more then available buffer size(%d)\n", length, DWC_OTG_HCD_DATA_BUF_SIZE);
		dev->status = 0;
		dev->act_len = done;
		return -1;
	}
	...
	while ((done < length) && !stop_transfer) {
		...
		memcpy(aligned_buffer, (char*)buffer + done, length - done);
